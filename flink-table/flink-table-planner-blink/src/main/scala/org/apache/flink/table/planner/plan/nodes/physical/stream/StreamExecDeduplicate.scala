...
import org.apache.flink.table.runtime.operators.deduplicate.{ProcTimeDeduplicateKeepFirstRowFunction, ProcTimeDeduplicateKeepLastRowFunction, ProcTimeMiniBatchDeduplicateKeepFirstRowFunction, ProcTimeMiniBatchDeduplicateKeepLastRowFunction, RowTimeDeduplicateKeepFirstRowFunction, RowTimeDeduplicateKeepLastRowFunction, RowTimeMiniBatchDeduplicateKeepFirstRowOperator, RowTimeMiniBatchDeduplicateKeepLastRowOperator}
...
class StreamExecDeduplicate(
  ...
  override protected def translateToPlanInternal(
    ...
    val keyRowType = RowType.of(keyFieldTypes: _*)
    val keyTypeInfo = InternalTypeInfo.of(keyRowType)
    ...
    val operator = if (isRowtime) {
      if(isMiniBatchEnabled) {
        ...
        if (keepLastRow) {
          new RowTimeMiniBatchDeduplicateKeepLastRowOperator(
            rowTypeInfo,
            rowSerializer,
            rowtimeIndex,
          ...
          new RowTimeMiniBatchDeduplicateKeepFirstRowOperator(
            rowTypeInfo,
            rowSerializer,
            rowtimeIndex,
            generateUpdateBefore,
        ...
        val processFunction = if (keepLastRow) {
          ...
          new RowTimeDeduplicateKeepFirstRowFunction(rowTypeInfo, rowtimeIndex,minRetentionTime)
        }
