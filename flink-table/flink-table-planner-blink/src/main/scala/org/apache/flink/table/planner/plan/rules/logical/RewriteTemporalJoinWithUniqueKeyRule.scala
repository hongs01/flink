...
/**
  ...
  * table join requires primary key and  row time attribute of temporal table. The temporal
  * table could be a table source or a view only if it contains the unique key and time attribute.
  *
  ...
  */
class RewriteTemporalJoinWithUniqueKeyRule extends RelOptRule(
  operand(classOf[FlinkLogicalJoin],
    operand(classOf[FlinkLogicalRel], any()),
    operand(classOf[FlinkLogicalSnapshot],
      operand(classOf[FlinkLogicalRel], any()))),
  "TemporalJoinRewriteUniqueKeyRule")
      ...
      override def visitCall(call: RexCall): RexNode = {
        ...
        isRowTimeTemporalTableJoin(snapshot)) {
          ...
          if (primaryKeyInputRefs.isEmpty) {
            throw new TableException("Event-Time Temporal Table Join requires both" +
              s" primary key and row time attribute in temporal table," +
...
object RewriteTemporalJoinWithUniqueKeyRule {
  val INSTANCE: RewriteTemporalJoinWithUniqueKeyRule = new RewriteTemporalJoinWithUniqueKeyRule
}
