 ...
 */
public class BaseRowCsvInputFormat extends RichInputFormat<BaseRow, FileInputSplit> {
	...
	public BaseRowCsvInputFormat(
		...
		TypeInformation[] csvFieldTypes = csvFieldNames.stream()
		...
		List<String> csvSelectFieldNames = selectFieldNames.stream()
				.filter(name -> !partitionKeys.contains(name)).collect(Collectors.toList());
		this.csvSelectConverters = csvSelectFieldNames.stream()
				.map(name -> fieldTypes.get(fieldNames.indexOf(name)))
		...
		this.inputFormat = new RowCsvInputFormat(null, csvFieldTypes, csvSelectFields);
	...
	public void open(FileInputSplit split) throws IOException {
		...
		for (int i = 0; i < selectFields.length; i++) {
			...
			if (partitionKeys.contains(name)) {
				this.row.setField(
						i, convertStringToInternal(partSpec.get(name), fieldTypes.get(selectField)));
			}
		}
